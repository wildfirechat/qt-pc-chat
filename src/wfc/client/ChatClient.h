...
#include "../message/MarkUnreadMessageContent.h"
...
Q_DECLARE_METATYPE(WFCLib::Message);
Q_DECLARE_METATYPE(WFCLib::UserInfo);
Q_DECLARE_METATYPE(WFCLib::GroupInfo);
#endif
...
public:
    ...
    virtual void onSuccess(int dataType) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const std::string &value) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const std::list<Message> &messages) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
/*
 获取消息回调
 */
class GetMessagesCallback {
public:
    GetMessagesCallback() {}
    virtual ~GetMessagesCallback() {}
...
public:
    ...
    virtual void onSuccess(int dataType, int64_t messageUid, int64_t timestamp) = 0;
    // 上传进度的回调，注意仅当媒体内容大于300K才会有回调
    virtual void onPrepared(int dataType, int messageId, int64_t timestamp) = 0;
    virtual void onProgress(int dataType, int uploaded, int total) = 0;
    virtual void onUploaded(int dataType, const std::string &remoteUrl) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const std::list<UserInfo> &messages) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const ChatroomInfo &chatroomInfo) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const ChatroomMemberInfo &chatroomMemberInfo) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const ChannelInfo &channelInfo) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const std::list<ChannelInfo> &channels) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
public:
    ...
    virtual void onSuccess(int dataType, const std::string &remoteUrl) = 0;
    // 上传进度的回调，注意仅当媒体内容大于300K才会有回调
    virtual void onProgress(int dataType, int uploaded, int total) = 0;
    virtual void onFailure(int dataType, int errorCode) = 0;
};
...
/*
 群组头像生成器。members可以传最多9个成员，实现方要来拼接头像。
 */
class DefaultPortraitProvider {
public:
    DefaultPortraitProvider() {}
    virtual std::string groupDefaultPortrait(const GroupInfo &groupInfo, const std::list<UserInfo> &members) = 0;
    virtual std::string userDefaultPortrait(const UserInfo &userInfo) = 0;
...
signals:
    ...
    void recalledMessage(const std::string &operatorId, int64_t messageUid);
    void deletedMessage(int64_t messageUid);
    void userInfoUpdated(const std::list<UserInfo> &userInfos);
    void groupInfoUpdated(const std::list<GroupInfo> &groupInfos);
    void groupMemberUpdated(const std::string &groupId);
    void contactUpdated(const std::list<std::string> &friendUids);
    ...
    void messageSendUploaded(long messageId, std::string remoteUrl);
    void messageSendFailure(long messageId, int errorCode);
    void draftUpdated(const Conversation &conversation, const std::string &draft);
    void unreadStatusCleared(const Conversation &conversation);
    void unreadStatusAllCleared();

    void conversationMessageCleared(const Conversation &conversation, int64_t before);
...
public:
    ...
    */
    ...
    /**
     ...
     @param top 是否置顶
     @param callback 结果回调
     @param dataType 回调的数据类型
     */
    void setConversationTop(const Conversation &conversation, int top, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param dataType 回调的数据类型
     */
    void setConversationSilent(const Conversation &conversation, bool silent, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    */
     ...
     * @param conversation 会话
     * @return 最早一条未读消息ID，如果没有未读消息，返回0
     */
    ...
    /**
     ...
     @param conversation 会话
     @param sync 是否同步到其他客户端
     */
    bool markAsUnread(const Conversation &conversation, bool sync);
    ...
    /**
     ...
     @return 消息实体
     */
    void getMessagesV2(const Conversation &conversation, const std::list<int> &contentTypes, int64_t fromIndex,int count, const std::string &user, GetMessagesCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @return 消息实体
     */
    const std::list<Message> getMessagesByMessageStatus(const std::list<int> &conversationTypes, const std::list<int> &lines, const std::list<MessageStatus> &messageStatuss, int64_t fromIndex, int count, const std::string &user);
    ...
    /**
     ...
     @param objectDataType 会话的类型

     @discuss 服务器不检查可撤回时间，应用逻辑来处理。
     */
    void recallMessage(int64_t messageUid, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @return 本地的用户信息，可能为空
     */
    const UserInfo getUserInfo(const std::string &userId, bool refresh = false);
    ...
    /**
     ...
     @return 本地的用户信息，可能为空
     */
    const UserInfo getUserInfo(const std::string &userId, const std::string &groupId, bool refresh = false);
    ...
    /**
     获取单条好友请求
     @param userId 用户ID
     @param incomming 是收到请求或者发出请求

     @return 好友请求
     */
    const FriendRequest getFriendRequest(const std::string &userId, bool incomming);
    ...
    /**
     ...
     @param callback 回调
     */
    void deleteFriend(const std::string &userId, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
	 ...
	 @param callback 回调
     */
    void sendFriendRequest(const std::string &userId, const std::string &reason, const std::string &extra, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void handleFriendRequest(const std::string &userId, bool accpet, const std::string &friendExtra, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    */
    ...
    /**
     ...
     @return 黑名单列表的用户ID
     */
    const std::list<std::string> getBlackList(bool refresh = false);
    ...
    /**
     ...
     @param callback 回调
     */
    void setBlackList(const std::string &userId, bool isBlackListed, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @return 群成员信息
     */
    const std::list<GroupMember> getGroupMembers(const std::string &groupId, bool forceUpdate = false);
    ...
    /**
     ...
     @return 群成员信息
     */
    const std::list<GroupMember> getGroupMembersByType(const std::string &groupId, int memberType);
    ...
    /**
     ...
     @return 群成员信息
     */
    const std::list<GroupMember> getGroupMembersByCount(const std::string &groupId, int count);
    ...
    /**
     ...
     @return 群信息
     */
    const GroupInfo getGroupInfo(const std::string &groupId, bool refresh = false);
    ...
    /**
     ...
     @param callback 回调
     */
    void createGroup(const std::string &groupId, const std::string &groupName, const std::string &groupPortrait, GroupType type, const std::string &groupExtra, const std::list<std::string> &groupMembers, const std::string &memberExtra, const std::list<int> &notifyLines = {0}, GeneralStringCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void addMembers(const std::list<std::string> &members, const std::string &groupId, const std::string &memberExtra, const std::list<int> &notifyLines = {0}, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void kickoffMembers(const std::list<std::string> &members, const std::string &groupId, const std::list<int> &notifyLines = {0}, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void quitGroup(const std::string &groupId, const std::list<int> &notifyLines = {0}, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void dismissGroup(const std::string &groupId, const std::list<int> &notifyLines = {0}, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void modifyGroupInfo(const std::string &groupId, ModifyGroupInfoType type, const std::string &newValue, const std::list<int> &notifyLines = {0}, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void transferGroup(const std::string &groupId, const std::string &newOwner, const std::list<int> &notifyLines = {0},  GeneralVoidCallback *callback = NULL, int callbackPara = 0);
     ...
     */
    void setUserSetting(UserSettingScope scope, const std::string &key, const std::string &value, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    */
    ...
    /**
     ...
     @param callback 回调
     */
    void setGlobalSilent(bool slient, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    */
    ...
    @param callback 回调
    */
    void setHiddenNotificationDetail(bool hidden, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    @param groupId 群组ID
    */
    bool isHiddenGroupMemberName(const std::string &groupId);
    ...
    @param callback 回调
    */
    void setHiddenGroupMemberName(bool hidden, const std::string &groupId, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    @param callback 回调
    */
    void joinChatroom(const std::string &chatroomId, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    @param callback 回调
    */
    void quitChatroom(const std::string &chatroomId, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    @param callback 回调
    */
    void getChatroomInfo(const std::string &chatroomId, int64_t updateDt, GetChatroomInfoCallback *callback = NULL, int callbackPara = 0);
    ...
    */
    ...
    /**
     ...
     @param callback 回调
     */
    void createChannel(const std::string &channelName, const std::string &channelPortrait, const std::string &desc, const std::string &extra, GetChannelInfoCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @return 群信息
     */
    ChannelInfo getChannelInfo(const std::string &channelId, bool refresh = false);
    ...
    /**
     ...
     @param callback 回调
     */
    void modifyChannelInfo(const std::string &channelId, ModifyChannelInfoType type, const std::string &newValue, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void searchChannel(const std::string &keyword, SearchChannelCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void listenChannel(const std::string &channelId, bool listen, GeneralVoidCallback *callback = NULL, int callbackPara = 0);
    ...
    /**
     ...
     @param callback 回调
     */
    void destoryChannel(const std::string &channelId, GeneralVoidCallback *callback = NULL, int callbackPara = 0);

    /**
     * @brief getAuthorizedMediaUrl
     * @param messageId
     * @param mediaType
     * @param mediaPath
     * @param callback
...
public:
    ...
    void onReceiveMessages(const std::list<Message> &messages, bool hasMore);
    void onRecallMessage(const std::string &operatorId, int64_t messageUid);
    void onDeleteMessage(int64_t messageUid);
    void onUserInfoUpdated(const std::list<UserInfo> &userInfos);
    void onGroupInfoUpdated(const std::list<GroupInfo> &groupInfos);
    void onGroupMemberUpdated(const std::string &groupId);
    void onContactUpdated(const std::list<std::string> &friendUids);
    ...
    void onMessageSendProgress(long messageId, int uploaded, int total);
    void onMessageSendUploaded(long messageId, const std::string &remoteUrl);
    void onMessageSendFailure(long messageId, int errorCode);

    void onVoidConextSuccess(void* pObj);
};
};
