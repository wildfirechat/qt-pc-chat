...
UserInfo::UserInfo(const UserInfo &a) {
    ...
    social = a.social;
    extra = a.extra;
    friendAlias = a.friendAlias;
    groupAlias = a.groupAlias;
    //0 normal = a.; 1 robot = a.; 2 thing = a.;
    type = a.type;
    deleted = a.deleted;
    updateDt = a.updateDt;
}

const std::string UserInfo::getReadableName() const
{
    if(!friendAlias.empty()) return friendAlias;
    if(!groupAlias.empty()) return groupAlias;
