...
const QuitGroupNotificationContentPrototype QuitGroupNotificationContent::sPrototype;
...
std::string QuitGroupNotificationContent::digest(const Message &message) const {
    return getNotificationUserName(operatorId, message.conversation.target) + " 退出了群组";
}
