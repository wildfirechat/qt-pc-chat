...
#include "shotarea/areacreatetool.h"
#include "shotarea/areamovetool.h"
#include "shotarea/areahandletool.h"
#include "movetool.h"
...
void Workspace::loadResource()
{
    ...
    if(GParams::instance()->scale() != 1.0)
    {
        ...
        rx.setPatternOptions(QRegularExpression::DotMatchesEverythingOption | QRegularExpression::UseUnicodePropertiesOption);
        int index = 0;

        QRegularExpressionMatch match;
        while ((match = rx.match(qss, index)).hasMatch()) {
            QString matchedText = match.captured(0);
            int capLen = matchedText.length() - 2;
