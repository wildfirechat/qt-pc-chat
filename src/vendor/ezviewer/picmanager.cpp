...
PicManager::PicManager(QWidget *parent)
    : ImageViewer(parent), curImage(ImageFactory::getImageWrapper(QString())),
...
//    state = NoFileNoPicture;
    ...
    connect(ImageLoader::getInstance(), &ImageLoader::imageLoaded, this, &PicManager::onImageLoaded);
}
...
void PicManager::updateFileNameList(const QString &curfile)
{
    ...
    list = QDir(curDir, IVConfig::supportFormats(), IVConfig::DefaultDirSortFlag, QDir::Files)
            .entryList();
...
{
    curImage->recycle();
    curImage = ImageFactory::getImageWrapper(QString());
    curPath = curName = curUrl = QString();
//    state = NoFileNoPicture;
    loadImage(QImage());
...
void PicManager::openImageMessage(const WFCLib::Message &imageMessage)
{
    WFCLib::ImageMessageContent *imgCnt = (WFCLib::ImageMessageContent *)imageMessage.content;

    curConversation = imageMessage.conversation;
    curImgCnt = imgCnt;
    currentMessageId = imageMessage.messageId;

    QString localPath;
    if(!imgCnt->localPath.empty()) {
        QFile file(imgCnt->localPath.c_str());
        if(file.exists()) {
            localPath = imgCnt->localPath.c_str();
            openFile(localPath);
            return;
        }
    }
    QString prefix = "images";
    if(ImageLoader::getInstance()->containDiskCache(imgCnt->remoteUrl.c_str(), prefix)) {
        openFile(ImageLoader::getInstance()->getDiskCache(imgCnt->remoteUrl.c_str(), prefix));
        return;
    } else {
        ImageLoader::getInstance()->loadUrl(imgCnt->remoteUrl.c_str(), -1, -1, prefix);

        curPath = "";
        curName = "";
        curUrl = imgCnt->remoteUrl.c_str();

        curImage->recycle();
        QImage image;
        if (image.loadFromData(QByteArray::fromStdString(imgCnt->thumbnail), "JPEG")) {
            loadImage(image, "");
        } else {
...
{
    if(!curConversation.target.empty()) {
        std::list<WFCLib::Message> messages = WFCLib::ChatClient::Instance()->getMessages(curConversation, {WFCLib::MESSAGE_CONTENT_TYPE_IMAGE}, currentMessageId, 1, "");
        if(!messages.empty()) {
            openImageMessage(messages.front());
            return true;
        }
        return false;
    }
...
void PicManager::onImageLoaded(const QString &url, int row, int column, const QString &prefix)
{
    if(curUrl == url) {
        QString prefix = "images";
        if(ImageLoader::getInstance()->containDiskCache(url, prefix)) {
            openFile(ImageLoader::getInstance()->getDiskCache(url, prefix));
            return;
        }
    }
}
