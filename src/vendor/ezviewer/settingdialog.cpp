...
QWidget *SettingDialog::creatAssociationWidget()
{
    ...
    if(OSRelated::isSupportAssociation()){
        ...
        QStringList formatList = IVConfig::formatsList();
        QCheckBox *cb;
        ...
        // future = QtConcurrent::run(this, &SettingDialog::reviewFileAssociation);
    }

    return assocWidget;
}

void SettingDialog::reviewFileAssociation()
{
...
void SettingDialog::reviewCheckAllButtonState()
{
    int size = gridLayout->count();
    QCheckBox *cb;
...
void SettingDialog::changeAssociation(bool enabled)
{
...
void SettingDialog::switchAllAssociation()
{
    QPushButton *button = dynamic_cast<QPushButton*>(sender());
    // if (button == checkAllButton) {
    //     future = QtConcurrent::run(this, &SettingDialog::changeAllAssociation, true);
    // } else if (button == uncheckAllButton) {
    //     future = QtConcurrent::run(this, &SettingDialog::changeAllAssociation, false);
    // }
...
void SettingDialog::changeAllAssociation(bool checked)
{
    int size = gridLayout->count();
    QCheckBox *cb;
    for(int i = 0; i < size; ++i){
