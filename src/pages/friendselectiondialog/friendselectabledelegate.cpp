...
// 自定义复选框代理类实现
FriendSelectableDelegate::FriendSelectableDelegate(QObject *parent) : QStyledItemDelegate(parent)
{
}

void FriendSelectableDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    // 计算复选框位置
    QRect checkBoxRect = QApplication::style()->subElementRect(QStyle::SE_CheckBoxIndicator, &opt);
    checkBoxRect.moveCenter(opt.rect.center());
    checkBoxRect.moveLeft(opt.rect.left() + 10);
    ...
    QRect iconRect(rect.left() + 10 + 20, rect.top() + 8, iconSize, iconSize);
    ...
    // 绘制文本
    QRect textRect(rect.left() + 10 + 20 + iconSize + 10, rect.top() + 8, rect.width()-(10 + 20 + iconSize + 10), iconSize);
...
bool FriendSelectableDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
...
QSize FriendSelectableDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
